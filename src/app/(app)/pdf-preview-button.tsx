    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-stretch sm:items-center sm:justify-center sm:p-6">
          <div className="bg-white w-full h-full sm:h-[95vh] sm:max-w-6xl sm:rounded-lg flex flex-col overflow-hidden">
            ...
            {/* #view=FitH tells the browser's built-in PDF viewer to fit the page
                to the iframe width instead of opening at a small default zoom. */}
            <iframe src={`${href}#view=FitH`} className="flex-1 w-full" title={filename} />
          </div>
        </div>
      )}
    </>
